...
func HeatmapProdiAngkatanHandler(c *gin.Context) {
	...
	var cohorts []Cohort
	DB.Model(&User{}).Select("prodi, angkatan, COUNT(*) as count").Where("role = ? AND prodi <> '' AND angkatan <> ''", RoleStudent).Group("prodi, angkatan").Scan(&cohorts)
	...
	var hAvgs []HAvg
	DB.Table("happiness_assessments h JOIN users u ON u.id = h.user_id").Select("u.prodi as prodi, u.angkatan as angkatan, AVG(h.happiness_index) as avg_hi").Where("u.role = ?", RoleStudent).Group("u.prodi, u.angkatan").Scan(&hAvgs)
	...
	var bAvgs []BAvg
	DB.Table("predictions p JOIN users u ON u.id = p.user_id").Select("u.prodi as prodi, u.angkatan as angkatan, AVG(p.burnout_score) as avg_burnout, SUM(CASE WHEN p.risk_level IN ('high','urgent','critical') THEN 1 ELSE 0 END) as high_risk").Where("u.role = ?", RoleStudent).Group("u.prodi, u.angkatan").Scan(&bAvgs)
	...
	prodiList := []string{}
	for p := range prodiSet { prodiList = append(prodiList, p) }
	angkatanList := []string{}
	for a := range angkatanSet { angkatanList = append(angkatanList, a) }
